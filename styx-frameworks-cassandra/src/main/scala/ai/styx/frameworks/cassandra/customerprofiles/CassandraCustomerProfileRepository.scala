...
import ai.styx.domain.{Balance, Customer}
...
class CassandraCustomerProfileRepository(repo: CassandraRepository) extends Logging {
  ...
  def getCustomerProfile(accountNumber: Int)(implicit ec: ExecutionContext): Future[Option[Customer]] = {
    ...
    repo.tryAsyncStatement(statement,
      ...
      .logFailure(t => LOG.error(s"FAILED to load customers for account number $accountNumber. Error message: ${t.getMessage}"))
      .logSuccess(customers =>
        if (customers.isEmpty)
          LOG.warn(s"No customers found for account number $accountNumber.")
        else if (customers.lengthCompare(1) > 0)
          LOG.warn(s"Too much customers found, expected 1, got ${customers.size}, taking the first.")
      ).map(_.headOption)
  }
    ...
    repo.tryAsyncStatement(statement,
      ...
      .logSuccess(initialBalances =>
        if (initialBalances.isEmpty)
          LOG.warn(s"No initial balance found for card $cardId")
      ).map(_.headOption)
  }
  ...
  def getTransactionSum(accountNumber: Int, cardId: String, maxDateTime: DateTime)(implicit ec: ExecutionContext): Future[Double] = {
    ...
    repo.tryAsyncStatement(statement,
      ...
      .logSuccess(transactionSums =>
        LOG.trace(s"${transactionSums.size} transactions found for for account number $accountNumber and card $cardId till $maxDateTime")
      ).map(_.sum)
  }
  ...
  def addTransaction(accountNumber: Int, cardId: String, dateTime: DateTime, amount: Double)(implicit ec: ExecutionContext): Future[Boolean] = {
    ...
    repo.tryAsyncStatement(statement,
      ...
      .logFailure(t => LOG.error(s"FAILED to add transaction for account number $accountNumber and cardId: $cardId. Error message: ${t.getMessage}"))
      .map(_.wasApplied())
  }
  ...
  def setCepParameters(periodInMinutes: Int, eventCount: Int, balanceThreshold: Int)(implicit ec: ExecutionContext): Future[Boolean] = ??? // TODO
}
