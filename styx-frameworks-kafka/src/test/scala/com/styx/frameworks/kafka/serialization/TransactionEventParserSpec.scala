...
class TransactionEventParserSpec extends BaseSpec {
  ...
  val generatedTransactionEvent: BaseTransactionEvent = parser.map(testKafkaEvent).get

  "TransactionEventParser" should "return a transaction event." in {
