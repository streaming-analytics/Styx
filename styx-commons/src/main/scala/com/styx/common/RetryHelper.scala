...
object RetryHelper extends Logging {
  ...
  def retryExecute[T](retries: Int = 0, sleepTime: Int = 1000)(command: => T): Try[T] = {
    Try {
      ...
      case e: Throwable =>
        if (retries == 0) {
          logger.warn(s"Got exception, $retries retries left, so throwing exception ${e.getMessage}.")
          Failure(e)
        } else {
          logger.warn(s"Got exception, $retries retries left, so sleeping $sleepTime: ${e.getMessage}.")
